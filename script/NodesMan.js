...
function reloadChart(id, time) {
    var node = d3.selectAll(".node").filter(function (d) {
        return d.id === id;
    });
    var path = node.selectAll("path");
    path.data(preparePieData(node.data()[0], 0));
    path.transition().duration(time).attrTween("d", arcTween);
};

function getNodeById(nodeId) {
    var result = d3.selectAll(".node").filter(function (d, i) {
        return d.id === nodeId;
    });
...
function NodesManeger() {
    ...
    this.getNodeById = function (nodeId) {
        var result = this.nodes.filter(function (d, i) {
            return d.id === nodeId;
        });
        return result.data()[0];
    };
    ...
    this.addPieChartsToNodes = function () {
        ...
        nodes1 = this.nodes.filter(function (d) {
            return d.id != "CreditWorthiness";
        });
        nodes2 = this.nodes.filter(function (d) {
            return d.id === "CreditWorthiness";
        });
        nodes1.selectAll("path")
            .data(function (d, i) {
                return preparePieData(d, i)
            })

            .enter()
            .append("svg:path")
            .attr("class", "piechart")
            //.attr("fill", function(d, i) {  return getColor(); })
    ...
    this.deleteChart = function (id) {
        ...
        var nodes1 = d3.selectAll(".node").filter(function (d) {
            return d.id === id;
        });
